...
- **Gated audited downloads.** Members of a resource's download group can export the current filtered results (CSV or XLSX) with a required data-handling acknowledgement and justification. Every download writes one audit row to `{APP_CATALOG}.{APP_SCHEMA}.download_audit` before the file is returned — if the audit fails, the download is blocked.
- **Admin console.** Members of `ADMIN_GROUP` manage resources and resource collections at `/admin` (query preview, column builder with per-column aggregation, add/**delete** resources, disclaimer, audit log). Each query-resource page can reveal the exact SQL it runs ("View SQL").
...
tests/                  pytest unit tests (pure functions, no SDK required, 347 passed)
docs/
